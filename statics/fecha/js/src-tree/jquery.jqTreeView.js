...
"use strict";
	...
	function TreeView(element, options) {
		...
		this.self = $(element).find("ul:eq(0)");				
	...
	TreeView.prototype.renderRootElements = function() {
		$("#" + this.options.id)
			.prop("id", this.options.id +"_wrapper")
			.addClass("ui-widget ui-jqtreeview-wrapper")
			.css( { width: this.options.width, height: this.options.height } )
			.append( $('<ul/>').prop("id", this.options.id));	
	};
	
	TreeView.prototype.renderAccessLink = function() {
		$(document.body)
			.append(	$('<div/>')
							.css( { position: 'absolute', top: '-9999px', left:'-9999px;' })
							.append( $('<a  tabindex="1"/>')
								.prop("id", this.options.id + "_accessLink")
					  )
			);
	};
	
	TreeView.prototype.renderUniqueID = function() {
		if (this.options.uniqueID) {
			var form = $(document.body).find("form");
			if (form.length > 0) {				
				form.append("<input type='hidden' id='" + this.options.uniqueID + "' name='" + this.options.uniqueID + "'></input>");
			}
		}
	};
	...
	TreeView.prototype.serializeCheckedState = function() {		
		...
		for (var i=0; i < checkedNodes.length; i++) {
			nodes[nodes.length] = this.serializeNode(checkedNodes[i]);
		};
	...
	TreeView.prototype.serializeSelectedState = function() {
		...
		for (var i=0; i < selectedNodes.length; i++) {
			nodes[nodes.length] = this.serializeNode(selectedNodes[i]);
		};
	...
	TreeView.prototype.serializeExpandedState = function() {
		...
		for (var i=0; i < expandedNodes.length; i++) {
			nodes[nodes.length] = this.serializeNode(expandedNodes[i]);
		};
	...
	TreeView.prototype.getNodeByText = function(text) {
		var result = this.self.find(".ui-jqtreeview-item-text:contains('" + text + "'):eq(0)");
	...
	TreeView.prototype.getCheckedNodes = function() {
		...
		$.each(this.self.find("input:checked"), function(i, checkbox) {			
			nodes[nodes.length] = that.getNodeParentLiElement(checkbox);
		});
	...
	TreeView.prototype.getSelectedNodes = function() {
		...
		$.each(this.self.find(".ui-state-highlight"), function(i, node) {
			nodes[nodes.length] = that.getNodeParentLiElement(node);
		});

		return nodes;
	};

	TreeView.prototype.getExpandedNodes = function() {
		var nodes = [];
		var that = this;
